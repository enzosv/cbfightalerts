...
function getStrongestWeapon(char, weapons) {
  const playerElement = parseInt(char.trait, 10);
  var strongest = {"strength":0}
  const characterPower = CharacterPower(char.level);
  for(var i=0; i<weapons.length; i++){
    const weapon = weapons[i]
    const weaponElement = parseInt(WeaponElement[weapon.element], 10);
    const weaponMultiplier = GetTotalMultiplierForTrait(weapon, playerElement);
    const totalPower = (characterPower * weaponMultiplier) + weapon.bonusPower;
    const totalMultiplier = 1 + (0.075 * (weaponElement === playerElement ? 1 : 0));
    weapon.strength = totalPower*totalMultiplier;
    if(weapon.strength > strongest.strength) {
      strongest = weapon
    }
  }
  return strongest
